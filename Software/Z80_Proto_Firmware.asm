...
; Project: Z80 PROTO, Z80 computer in breadboard
; File: Firmware program (BIOS + Mini OS)
;===========================================================================
...
; Program setup & initialization
    LD SP, RAMBEGIN+RAMSIZE-1        ;Set stack pointer to the end of the ram

; Board initialization without RAM memory

; 8255 PIO setup
; Port A: Output, LCD data
; Port B: Input, keyboard read data
; Port C: Output, LCD control signals & keyboard multiplex (sweeping)
    LD A,82H               
    OUT (DSKY_IOSETUP),A                    ; Define 8255 mode and ports functions

; DSKY LCD setup
    LD A,00H
    OUT (DSKY_LCDE_RW_RS),A                 ; Reset E pin
    LD A,01H                                ; * CLEAR LCD *
    OUT (DSKY_LCDDATA),A                    ; Send LCD setup command
    LD A,04H
    OUT (DSKY_LCDE_RW_RS),A                 ; Set E pin
    LD B,5                                  ; Define delay for LCD to execute command
.ini_dly_clearlcd:
    DJNZ .ini_dly_clearlcd                  ; Delay for LCD to execute command
    LD A,00H
    OUT (DSKY_LCDE_RW_RS),A                 ; Reset E pin
    LD A,38H                                ; *LCD IN 8 BIT MODE *
    OUT (DSKY_LCDDATA),A                    ; Send LCD setup command
    LD A,04H
    OUT (DSKY_LCDE_RW_RS),A                 ; Set E pin
    LD B,5                                  ; Define delay for LCD to execute command
.ini_dly_eightbitmd:
    DJNZ .ini_dly_eightbitmd                ; Delay for LCD to execute command
    LD A,00H
    OUT (DSKY_LCDE_RW_RS),A                 ; Reset E pin
    LD A,80H                                ; * SETS CURSOR TO TOP LINE 1ST POSITION *
    OUT (DSKY_LCDDATA),A                    ; Send LCD setup command
    LD A,04H
    OUT (DSKY_LCDE_RW_RS),A                 ; Set E pin
    LD B,5
.ini_dly_firstpos:
    ...
    LD A,00H
    OUT (DSKY_LCDE_RW_RS),A                 ; Reset E pin
    LD A,0CH                                ; * SETS DISPLAY ON, CURSOR OFF, NO BLINKING *
    OUT (DSKY_LCDDATA),A                    ; Send LCD setup command
    LD A,04H
    OUT (DSKY_LCDE_RW_RS),A                 ; Set E pin
    LD B,5
.ini_dly_cursormd:
    ...
    LD A,00H
    OUT (DSKY_LCDE_RW_RS),A                 ; Reset E pin
    LD A,06H                                ; * SETS CURSOR MOVING RIGHT AND NO TEXT SHIFTING *
    OUT (DSKY_LCDDATA),A                    ; Send LCD setup command
    LD A,04H
    OUT (DSKY_LCDE_RW_RS),A                 ; Set E pin
    LD B,5
...
.ini_loop:
    ...
    OUT (DSKY_LCDDATA),A                    ; Send LCD setup command
    LD A,05H
    OUT (DSKY_LCDE_RW_RS),A                 ; Set E and RS pins
    LD A,00H
    OUT (DSKY_LCDE_RW_RS),A                 ; Reset E and RS pins
...
; Delay (operative, ms) for clk = 4 MHz ;
...
.delay_mult:
    LD A,B
    LD B,255                ;Number of 1st. loop cycle adjusted to A = delay ms with minimum error
.delay_1ms1:
    DJNZ .delay_1ms1
    LD B,51                 ;Number of 2nd. loop cycle adjusted to A = delay ms with minimum error
.delay_1ms2:
    DJNZ .delay_1ms2
    LD B,A
    DJNZ .delay_mult
